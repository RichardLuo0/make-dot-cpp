...
#define GENERATE_COMPILE_METHOD(NAME, ARGS, PASS_ARGS) \
  ...
  GENERATE_COMPILE_METHOD(compile,
                          ...
                          (input, output, isDebug, modulePath, extraOptions)) {
    return std::format(
        "{} -fansi-escape-codes -fcolor-diagnostics -c {} {} {} {} {} -o {}",
        Process::findExecutable("clang++"), (isDebug ? "-g" : ""),
        compileOptions, extraOptions,
        modulePath.has_value()
            ? "-fprebuilt-module-path=" + modulePath.value().generic_string()
            : "",
  ...
  GENERATE_COMPILE_METHOD(compilePCM,
                          (const Path &input, const Path &output,
                           const Path &modulePath,
                           const std::string &extraOptions = ""),
                          (input, output, modulePath, extraOptions)) {
    return std::format(
        "{} -fansi-escape-codes -fcolor-diagnostics "
        "-std=c++20 --precompile -c {} {} -fprebuilt-module-path={} {} -o {}",
        Process::findExecutable("clang++"), compileOptions, extraOptions,
        modulePath.generic_string(), input.generic_string(),
...
#undef GENERATE_COMPILE_METHOD
  ...
  ModuleInfo getModuleInfo(const Path &input) const override {
    ModuleInfo info;
    const auto result =
        Process::run(Process::findExecutable("clang-scan-deps") +
                     " -format=p1689 -- " + compileCommand(input, "test"));
    if (result.status != 0) throw ScanDepsError(input);
